...
inline double& vec2::operator [](int i) {
    if ((i < VX) || (i > VY)) throw range_error("Index out of bounds");
    return n[i];
}
...
inline double& vec3::operator [](int i) {
    if ((i < VX) || (i > VZ)) throw range_error("Index out of bounds");
    return n[i];
}
...
inline mat3 mat3::inverse() const // Gauss-Jordan elimination with partial pivoting
{
    ...
    for (j = 0; j < 3; j++) { // Find largest pivot in column j among rows j..2
        ...
        if (a.v[j].n[j] == 0.) {
			throw domain_error("mat3::inverse: singular matrix; can't invert");
		}
        b.v[j] /= a.v[j].n[j];
        a.v[j] /= a.v[j].n[j];
...
//inline mat4::mat4(float * rowMajorArray)
//{
//	mat4(vec4(rowMajorArray[0], rowMajorArray[1], rowMajorArray[2], rowMajorArray[3]),
//		vec4(rowMajorArray[4], rowMajorArray[5], rowMajorArray[6], rowMajorArray[7]),
//		vec4(rowMajorArray[8], rowMajorArray[9], rowMajorArray[10], rowMajorArray[11]),
//		vec4(rowMajorArray[12], rowMajorArray[13], rowMajorArray[14], rowMajorArray[15]));
//}
...
inline vec4& mat4::operator [](int i) {
    if ((i < VX) || (i > VW)) throw range_error("Index out of bounds");
    return v[i];
}
...
inline mat4 mat4::inverse() const // Gauss-Jordan elimination with partial pivoting
{
    ...
    for (j = 0; j < 4; j++) { // Find largest pivot in column j among rows j..3
        ...
        // Scale row j to have a unit diagonal
        if (a.v[j].n[j] == 0.){
			throw domain_error("mat4::inverse: singular matrix; can't invert\n");
		}
        b.v[j] /= a.v[j].n[j];
        a.v[j] /= a.v[j].n[j];
...
inline mat4 getPerspectiveMatrix(double fovy, double ar, double n, double f) {
	double halfFOV = 0.5 * fovy * (M_PI / 180.0);
...
inline mat4 getPerspectiveMatrix(double l, double r, double b, double t, double n, double f) {
	return mat4(
...
inline mat4 getOrthographicMatrix(double l, double r, double b, double t, double n, double f) {
	return mat4(
		vec4(2/(r-l), 0, 0, (l+r)/(l-r)),
		vec4(0, 2/(t-b), 0, (b+t)/(b-t)),
		vec4(0, 0, 2/(n-f), (n+f)/(n-f)),
		vec4(0, 0, 0, 1));
}

inline mat4 getLookAtCameraMatrix(const vec3& eyePos, const vec3& eyeTarget, const vec3& eyeUp) {
	vec3 w = eyeTarget - eyePos;
	w.normalize();
	vec3 u = w ^ eyeUp;
	u.normalize();
	vec3 v = u ^ w;
	...
	return mat4(vec4(u, 0), vec4(v, 0), vec4(-w, 0), vec4(eyePos, 1)).transpose();
}
